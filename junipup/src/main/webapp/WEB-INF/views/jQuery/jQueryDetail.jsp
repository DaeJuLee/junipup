...
<table cellspacing="0" border="1" class="table" align="center">
	...
	<tbody> 
		<tr>
			<th scope="row">작성자</th>
			<td>${jQueryDetail.nickname }</td>
			<th scope="row">작성일</th>
			<td>${jQueryDetail.b_date }</td>
			<th scope="row">등급</th>
			<td>${jQueryDetail.mrank }</td>
		</tr>
		<tr>
			<th scope="row">아이피</th>
			<td>${jQueryDetail.ip }</td>
			<th scope="row">파일크기</th>
			<td> ${jQueryDetail.filesize }</td>
			<th scope="row">파일이름</th>
			<td>${jQueryDetail.filename }</td>			
		</tr>
		<tr>
		<td colspan="6">
		<div class="contents-wrap">
		내용 : ${jQueryDetail.content }
		</div>
		</td>
		</tr>	
	</tbody>
...
<script type="text/javascript">
	var onModify = function(){
		var form = $('.viewForm')[0];
		form.action = 'jQueryUpdateForm.do';
		form.submit();
	};
	
	var onList = function(){
		location.href='jQueryMain.do';
	};
