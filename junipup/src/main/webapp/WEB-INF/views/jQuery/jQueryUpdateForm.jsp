...
<form action="jspUpdate.do" id="boardWriteForm" method="post"> 
	...
	<colgroup>
		...
		<tr><th>No.</th>
		<td><input type="hidden" name="bnum" value="${jQueryUpdateForm.bnum }">
			${jQueryUpdateForm.bnum }</td>
		</tr>
		<tr><th>제목</th>
			<td><input type="text" name="title" value="${jQueryUpdateForm.title }" required="required"></td></tr>
		<tr><th>별명</th>
			<td><input type="hidden" name="nickname" value="${jQueryUpdateForm.nickname }">${jQueryUpdateForm.nickname }</td></tr>
		<tr><th>등급</th>
			<td><input type="hidden" name="mrank" value="${jQueryUpdateForm.mrank }">${jQueryUpdateForm.mrank }</td></tr>
		<tr><th>카테고리</th>
		...
		<tr><th>내용</th>
			...
			name="content" required="required" style="width:100%;">${jQueryUpdateForm.content }</textarea></div></td></tr>
	</table>
...
var oEditors = [];
	...
	fOnAppLoad: function () { 
        //수정모드를 구현할 때 사용할 부분. 로딩이 끝난 후 값이 체워지게 하는 구현을 하면 된다.
         var title = '${jQueryUpdateForm.title}';               
         var content = '${jQueryUpdateForm.content}';         //db에서 불러온 값을 여기에서 체워넣으면 됨.
...
var onModify = function(){
	...
	boardWriteForm.action ="jQueryUpdate.do";              
