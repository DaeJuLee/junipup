...
<body> 
<div class="container">
<form action="javaUpdate.do" id="boardWriteForm" method="post"> 
	...
	<colgroup>
		...
		<tr><th>No.</th>
		<td><input type="hidden" name="bnum" value="${javaDetail.bnum }">
			${javaDetail.bnum }</td>
		</tr>
		<tr><th>제목</th>
			<td><input type="text" name="title" value="${javaDetail.title }" required="required"></td></tr>
		<tr><th>별명</th>
			<td><input type="hidden" name="nickname" value="${javaDetail.nickname }">${javaDetail.nickname }</td></tr>
		<tr><th>등급</th>
			<td><input type="hidden" name="mrank" value="${javaDetail.mrank }">${javaDetail.mrank }</td></tr>
		<tr><th>카테고리</th>
		...
		<tr><th>내용</th>
			...
			name="content" required="required" style="width:100%;">${javaDetail.content }</textarea></div></td></tr>
	</table>
	<input type="button" value="목록보기" 
		onclick="location.href='javaMain.do'" class="button">
	<div class="buttonDiv">
...
var oEditors = [];
	...
	fOnAppLoad: function () { 
        //수정모드를 구현할 때 사용할 부분. 로딩이 끝난 후 값이 체워지게 하는 구현을 하면 된다.
         var title = '${javaDetail.title}';               
         var content = '${javaDetail.content}';         //db에서 불러온 값을 여기에서 체워넣으면 됨.
...
var onWrite = function(){
	...
	boardWriteForm.action ="javaInsert.do";              
...
var onModify = function(){
	oEditors.getById["txtContent"].exec("UPDATE_CONTENTS_FIELD", []); // 에디터의 내용이 textarea에 적용됨
	var boardWriteForm = document.getElementById("boardWriteForm");  
	boardWriteForm.action ="javaUpdate.do";              
	boardWriteForm.submit();  
};
