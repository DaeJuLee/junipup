...
<table cellspacing="0" border="1" class="table" align="center">
	...
	<form class="viewForm" method="post">
			<input type="hidden" name="bnum" value="${javaDetail.bnum }" />
			...
			<c:otherwise>
				<c:choose>							
					<c:when test="${USER_KEY.nickname == javaDetail.nickname  }">
...
<script type="text/javascript">
	var onModify = function(){
		var form = $('.viewForm')[0];
		form.action = 'javaUpdateForm.do';
		form.submit();
	};
	
	var onList = function(){
		location.href='javaMain.do';
	};
	
	var onDelete = function(){	
		var form = $('.viewForm')[0];
		form.action = 'javaDeleteCheck.do';
		form.submit();
	};
</script>
</html>