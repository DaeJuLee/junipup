...
<table cellspacing="0" border="1" class="table" align="center">
	...
	<tbody> 
		<tr>
			<th scope="row">작성자</th>
			<td>${noticeDetail.nickname }</td>
			<th scope="row">작성일</th>
			<td>${noticeDetail.b_date }</td>
			<th scope="row">등급</th>
			<td>${noticeDetail.mrank }</td>
		</tr>
		<tr>
			<th scope="row">아이피</th>
			<td>${noticeDetail.ip }</td>
			<th scope="row">파일크기</th>
			<td> ${noticeDetail.filesize }</td>
			<th scope="row">파일이름</th>
			<td>${noticeDetail.filename }</td>			
		</tr>
		<tr>
		<td colspan="6">
		<div class="contents-wrap">
		내용 : ${noticeDetail.content }
		</div>
		</td>
		</tr>	
	</tbody>
	...
	<form class="viewForm" method="post">	
			<input type="hidden" name="bnum" value="${noticeDetail.bnum }" />
...
<script type="text/javascript">
	var onModify = function(){
		var form = $('.viewForm')[0];
		form.action = 'noticeUpdateForm.do';
		form.submit();
	};
	...
	var onDelete = function(){	
		var form = $('.viewForm')[0];
		form.action = 'noticeDeleteCheck.do';
		form.submit();
	};
</script>
</html>