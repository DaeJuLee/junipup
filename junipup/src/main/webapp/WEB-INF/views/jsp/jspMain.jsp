...
<div class="tab_list m1" align="center">
    <ul class="tabs">
    <li class="tab1"><a href="jspMain.do"><span>전체게시판</span></a>
        <ul>
        <li class="more">› <a href="#">전체게시판 더보기</a></li>
        </ul>
    </li>
    <li class="tab2"><a href="jspMain.do?category=기초익히기"><span>기초익히기</span></a>
        <ul>
        <li class="more">› <a href="#">기초익히기 더보기</a></li>
        </ul>
    </li>

    <li class="tab3"><a href="jspMain.do?category=등급올리기"><span>등급올리기</span></a>
        <ul>
        <li class="more">› <a href="#">등급올리기 더보기</a></li>
        </ul>
    </li>

    <li class="tab4"><a href="jspMain.do?category=질문게시판"><span>질문게시판</span></a>
        <ul>       
        <li class="more">› <a href="#">질문게시판 더보기</a></li>
        </ul>
    </li>
    
    <li class="tab5"><a href="jspMain.do?category=응용게시판"><span>응용게시판</span></a>
        <ul>       
...
<c:set var="num" value="${pg.total - pg.start +1}" />
	...
	<c:set var="noticenum" value="${noticenum-1 }" />	
		...
		<c:forEach items="${list }" var="jsp">
			<tr>
				<td>${num }</td>
				<td>${jsp.category }</td>
				<td><a href="findMember.do?nickname=${jsp.nickname}">${jsp.nickname }</a></td>
	...
	<c:if test="${pg.startPage > pg.pageBlock}">		
		...
		<button type="button" class="button" onclick="location.href='jspMain.do?currentPage=${pg.startPage-pg.pageBlock }'">이전</button>		
	</c:if>

	<c:forEach var="pg" begin="${pg.startPage }" end="${pg.endPage }" step="1">
		<a href="jspMain.do?currentPage=${pg }"><span>${pg}</span></a>
	</c:forEach>
	...
	<c:if test="${pg.endPage < pg.totalPage }">		
		...
		<button type="button" class="button" onclick="location.href='jspMain.do?currentPage=${pg.startPage+pg.pageBlock}'">다음</button>
	</c:if>
	</div>
</body>
</html>