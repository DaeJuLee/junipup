				...
				if (this.document.body.createTextRange) {
					try {
						...
						if (this.document.selection) {
							...
							var d = this._findNextElement(this.window
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	_setRoundedCornerButton : function() {
		var e, g, d, c, a, f, b, h;
		f = jindo.$$(">ul>li[class*=" + this.sUIClassPrefix + "]:only-child",
				this.elTextTool);
		for (e = 0, g = f.length; e < g; e++) {
			d = f[e];
			c = jindo.$Element(d);
			if (c.hasClass("husky_seditor_ui_text_more")
					|| c.hasClass("husky_seditor_ui_photo_attach")
					|| c.hasClass("husky_seditor_ui_map_attach")) {
				continue
			}
			c.addClass("single_child");
			jindo.$Element(jindo.$$.getSingle("button>span._buttonRound", d))
					.addClass("tool_bg")
		}
		b = jindo.$$("ul>li[class*=" + this.sUIClassPrefix + "]:first-child",
				this.elTextTool);
		for (e = 0, g = b.length; e < g; e++) {
			d = b[e];
			c = jindo.$Element(d);
			if (c.hasClass("husky_seditor_ui_fontName")
					|| c.hasClass("husky_seditor_ui_text_more")
					|| c.hasClass("husky_seditor_ui_photo_attach")
					|| c.hasClass("husky_seditor_ui_map_attach")
					|| c.hasClass("single_child")) {
				continue
			}
		...
		h = jindo.$$("ul>li[class*=" + this.sUIClassPrefix + "]:last-child",
				this.elTextTool);
		for (e = 0, g = h.length; e < g; e++) {
			d = h[e];
			c = jindo.$Element(d);
			if (c.hasClass("husky_seditor_ui_fontSize")
					|| c.hasClass("husky_seditor_ui_text_more")
					|| c.hasClass("husky_seditor_ui_photo_attach")
					|| c.hasClass("husky_seditor_ui_map_attach")
					|| c.hasClass("single_child")) {
		...
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				if (jindo.$Agent().navigator().msafari) {
					jindo.$Element(this.oResizeGrip).first().text(
							"입력창 높이 자동 조정 바");
				...
				if (!!this.welNoticeLayer
					...
					this.welNoticeLayer.delegate("click", "BUTTON.bt_clse",
							jindo.$Fn(this._closeNotice, this).bind());
				...
				if (this.isUseVerticalResizer() === false
						&& this.oApp.isUseModeChanger() === false) {
					this.elModeToolbar.style.display = "none"
				}
			},
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo
		.$Class({
			...
			$ON_PASTE_HTML : function(b, g, e) {
				...
				/* if(b.indexOf("<img")>-1){l=c.startContainer;if(l.nodeType===1&&l.tagName==="P"){h=jindo.$Element(l).child(function(m){return(m.$value().nodeType===1&&m.$value().tagName==="IMG")},1);if(h.length>0){d=h[h.length-1].$value();k=d.nextSibling;while(k){a=k.nextSibling;if(k.nodeType===3&&(k.nodeValue==="&nbsp;"||k.nodeValue===unescape("%u00A0"))){l.removeChild(k)}k=a}}}} */if (!e) {
					this.oApp
		...
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				if (!this.htBrowser.safari
					...
					if (this.htBrowser.ie && this.htBrowser.nativeVersion <= 10
							&& this.htBrowser.version <= 10) {
						this.wfnBeforePasteHandler = jindo.$Fn(
								this._handleBeforePaste, this);
						this.wfnBeforePasteHandler.attach(this.elBody,
								"beforePaste")
					}
				}
			},
			...
			_saveOriginalContents : function() {
				...
				if (!this._sBM_IE) {
					...
					b.collapseToEnd();
					b.select();
					if (this.htBrowser.ie && this.htBrowser.nativeVersion == 11
							&& this.htBrowser.version == 11) {
					}
				}
			},
			...
			_filterPastedContents : function(ai) {
				...
				var ae = false, s = false, p = 0, aH = 0, y = false;
				var an, U, aj = [], r = -1, q, w = "", ac = [ "BLOCKQUOTE",
						"DD", "DIV", "DL", "FORM", "H1", "H2", "H3", "H4",
						"H5", "H6", "HR", "OL", "P", "TABLE", "UL", "IFRAME" ], v = new RegExp(
						"^(" + ac.join("|") + ")$", "i"), z = false, aA = 0, h = 0, k = [], L = 0;
				var R = 0;
				var E = [];
				var az = /^<[^?\/\s>]+(([\s]{0})|([\s]+[^>]+))>/, aD = /^<[\/]?([^\s]+)(([\s]{0})|([\s]+[^>]+))>/, M = /^<\/[A-Za-z]+>/, aB = /^<[^>]+\/>/, am = /^<!--[^<>]+-->/, O = /^<!--[^->]+>/, t = /^<![^->]+-->/, n = /^[\n\r\t\s]+/, aG = /^[^<\n]+/, av = /^<[^<>]+>/, J = /^<\/[^<>]+>/, ao = /(mso-[^:]+[\s]*:[\s]*)([^;"]*)([;]?)/gi, aC = /(style[\s]*=[\s]*)(["'])([^"']*)(["'])/i, C = /class[\s]*=[\s]*(?:(?:["']([^"']*)["'])|([^\s>]+))/i, ap = /(^<table)/gi, b, ay;
				var I = true, m = 0, G = true, A = true, aF = true, au = [
						"BR", "IMG", "COL", "HR" ], ag = new RegExp("^("
						+ au.join("|") + ")$", "i"), X, d = [ "SCRIPT", "STYLE" ], at = (d.length > 0) ? new RegExp(
						"^(" + d.join("|") + ")$", "i")
						: null, e = false, V = false, o = false, aK = 0, f = [], W = (f.length > 0) ? new RegExp(
						"^(" + f.join("|") + ")$", "i")
						: null, F = false, ad = false, j = false, Z = 0;
				while (ai != "") {
					U = "", an = "";
					X = true;
					if (aB.test(ai)) {
							...
							if (O.test(ai)) {
								...
								if (am.test(ai)) {
									...
									if (az.test(ai)) {
										...
										an = an.replace(/&quot;/gi, "'");
										an = an.replace(ao, "");
										var c = "", K = "", Q = "", aa = "", g = "", aE = "", x = false, H = false, ab = /([^\w-])(width[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)(["']?)/i, aI = /([^\w-])(height[\s]*=[\s]*)(["']?)([A-Za-z0-9.]+[%]?)(["']?)/i, ah = /(["';\s])(width[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i, l = /(["';\s])(height[\s]*:[\s]*)([A-Za-z0-9.]+[%]?)([;]?)/i;
										if (this.htBrowser.chrome
			...
			_loadToPasteHelper : function() {
				...
				try {
					if (this._aGoesPreviousParagraph.length > 0) {
						...
						for (var h = 0, l = p.length; h < l; h++) {
							this.elPasteHelper
									.appendChild(p[h].cloneNode(true))
						}
						if (!c) {
							if (j.length > 0) {
								if (!!j.splice) {
					...
					if (j.length > 0) {
						for (var h = 0, l = j.length; h < l; h++) {
							this.elPasteHelper
									.appendChild(j[h].cloneNode(true))
						}
					}
				} catch (n) {
					throw n
			...
			_loadToBody : function() {
				...
				try {
					...
					g.select();
					var p = g.getNodes();
					var c = this._markMatchedElementIndex(p,
					...
					n.select();
					var f = n.getNodes();
					var m = this._markMatchedElementIndex(f,
							this.aConversionTarget);
		...
		.$Class({
			...
			_assignHTMLElements : function(a) {
				...
				this.elNanummyeongjo = jindo.$$.getSingle(
						...
						this.oDropdownLayer);
				this.elNanumgothiccoding = jindo.$$.getSingle(
						"LI.husky_seditor_font_nanumgothiccoding",
						this.oDropdownLayer);
...
nhn.husky.SE2M_Hyperlink = jindo
		.$Class({
			...
			_validateTarget : function() {
				...
				if (b.ie) {
					jindo.$A(this.oSelection.getNodes(true)).forEach(
							function(c, d, e) {
								if (!!c
										&& c.nodeType == 1
										&& c.tagName.toLowerCase() == "iframe"
										&& c.getAttribute("s_type")
												.toLowerCase() == "db") {
									a = false;
									jindo.$A.Break()
								}
...
nhn.husky.SE2M_Quote = jindo
		.$Class({
			...
			_wrapBlock : function(D, H) {
				var y, q, u, x, c = /BODY|TD|LI/i, l, A, d, g, j, B, a, z, K, f, b, r, G, h, J, C;
				this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", [ "BLOCK QUOTE", {
					sSaveTarget : "BODY"
				} ]);
				y = this.oApp.getSelection();
				if (y.startContainer === y.endContainer) {
					if (y.startContainer.nodeType === 1
							&& y.startContainer.tagName === "P") {
						if (nhn.husky.SE2M_Utils.isBlankNode(y.startContainer)
								|| nhn.husky.SE2M_Utils.isFirstChildOfNode(
										"IMG", y.startContainer.tagName,
										y.startContainer)
								|| nhn.husky.SE2M_Utils.isFirstChildOfNode(
										"IFRAME", y.startContainer.tagName,
										y.startContainer)) {
						...
						var n = e.parentNode;
						var w = nhn.husky.SE2M_Utils.findAncestorByTagName("P",
								n);
						var v = [ "TD", "BODY" ];
						if (!w) {
							for (var G = 0, o = v.length; G < o; G++) {
								w = nhn.husky.SE2M_Utils.findAncestorByTagName(
										v[G], n);
								if (w) {
									break
								}
				...
				while (!g.tagName
						|| (g.tagName && g.tagName.match(/UL|OL|LI|IMG|IFRAME/))) {
					g = g.parentNode
				}
				...
				if (z && z.parentNode && z.parentNode.tagName == "BODY"
						&& !z.nextSibling) {
		...
		.$Class({
			...
			_assignHTMLObjects : function(b) {
				...
				this.oTextField = jindo.$$.getSingle("INPUT",
						this.elDropdownLayer);
				this.oInsertButton = jindo.$$.getSingle("BUTTON.se2_confirm",
						this.elDropdownLayer);
				...
				for (var p = 0; p < 2; p++) {
					for (var s = 0; s < d[p].length; s++) {
						...
						if (g.getComputedStyle) {
							...
							v = parseFloat(getComputedStyle(g).paddingBottom,
									10);
							o = parseFloat(getComputedStyle(g).borderLeftWidth,
									10);
							f = parseFloat(
									getComputedStyle(g).borderRightWidth, 10);
							e = parseFloat(getComputedStyle(g).borderTopWidth,
									10);
							z = parseFloat(
									getComputedStyle(g).borderBottomWidth, 10)
						} else {
							...
							o = this.parseBorder(b.css("borderLeftWidth"), b
									.css("borderLeftStyle"));
							f = this.parseBorder(b.css("borderRightWidth"), b
									.css("borderRightStyle"));
							e = this.parseBorder(b.css("borderTopWidth"), b
									.css("borderTopStyle"));
							z = this.parseBorder(b.css("borderBottomWidth"), b
			...
			_insertRowBelow : function() {
				...
				for (var q = this.htSelectionSPos.y; q <= this.htSelectionEPos.y; q++) {
					...
					if (c.ie && c.nativeVersion >= 9) {
						var p = h[this.htSelectionEPos.y].nextSibling
								.cloneNode();
						m.insertBefore(p, g);
...
nhn.husky.SE2M_ImgSizeRatioKeeper = jindo
		.$Class({
			...
			$LOCAL_BEFORE_FIRST : function() {
				...
				this.bIE = jindo.$Agent().navigator().ie;
				this.elCheckImgAutoAdjust = jindo.$$.getSingle(".se2_srate",
						this.elAppContainer);
				if (!this.bIE) {
